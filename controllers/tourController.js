...
exports.checkID = (req, res, next, val) => { //Creo metodo per controllo ID da utilizzare in un Param Middleware
...
//Handlers
exports.getTours = (req, res) => { //Assegno a costante funzione per recuperare tours
    res.status(200).json(
        {
            'status': 'success',
...
exports.getTour = (req, res) => { //Assegno a costante funzione per recuperare tour
    const id = req.params.id * 1; //Recupero id da url e lo trasformo in numero
    const tour = tours.find(el => el.id === id);
...
exports.createTour = (req, res) => { //Assegno a costante funzione per creare tour
